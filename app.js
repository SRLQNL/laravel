...
if(__LOCALHOST) http = require('http').createServer(app); else {
    http = require('https').createServer({
        key: fs.readFileSync('/etc/letsencrypt/live/playx777.life/privkey.pem', 'utf8'),
        cert: fs.readFileSync('/etc/letsencrypt/live/playx777.life/fullchain.pem', 'utf8')
    }, app);
}
var io = require('socket.io')(http), domain = __LOCALHOST ? 'https://playx777.life' : 'https://playx777.life';


var chat_history = [{"id":-1,"message":"Chat cleared successfully!","user_id":"0","avatar":"/avatar/5f08dade79789","name":"Система","type":"admin","skip":false,"level":10},];
...
setInterval(function () {
    requestify.get(domain+'/chatfakewithdraw/' + system_key)
        .then(function(response) {
            ...
            io.emit('chat message', JSON.stringify({
            'id': -1,
            'message': 'Successfully paid <span>'+parseFloat(response.sum).toFixed(2)+' $</span><br>toа <span>'+response.pay+'</span>',
            'user_id': response.user_id,
            'avatar': response.avatar,
            ...
            addToHistory({
            'id': -1,
            'message': 'Successfully paid <span>'+parseFloat(response.sum).toFixed(2)+' $</span><br>to <span>'+response.pay+'</span>',
            'user_id': response.user_id,
            'avatar': response.avatar,
            'name': response.username,
            'type': 'payout',
