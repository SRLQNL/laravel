...
class GeneralController extends Controller {
    ...
    public static function get_time($time) {
	 ...
	 public function task_description($id) {
        $task = Task::where('id', $id)->first();
        if($task == null) return 'La tarea no existe';
        switch ($task->game_id) {
            case 3: return 'Reach the coefficient valuex'.$task->value; break;
            case 4: return 'Guess the side '.$this->declOfNum($task->value, array('times', 'times', 'times')).' row'; break;
            case 5:
            case 9: return 'Open cells '.$this->declOfNum($task->value, array('times', 'times', 'times')).' row'; break;
            case 1: return 'Get number '.$task->value; break;
            case 7: return 'Guess the cards '.$this->declOfNum($task->value, array('times', 'times', 'times')).' row'; break;
            default: return 'N/A';
        }
    }
    ...
    public function sendEmail($to, $title, $html) {
        try {
            ...
            $mail->SMTPSecure = 'ssl';
            $mail->Port = 465;
            $mail->Username = "schtelman.andrei@yandex.ru";
            $mail->Password = "yfkgmyeqfjcdvydg";

            $mail->isHTML(true);
            $mail->Subject = $title;
            $mail->Body = CssInliner::fromHtml($html)->inlineCss()->render();

            $mail->setFrom('schtelman.andrei@yandex.ru', 'Test');
            $mail->addReplyTo('schtelman.andrei@yandex.ru', 'Test');
    ...
    public function blackjack_get_winner($id) {
        ...
        if($playerScore == $dealerScore) {
            ...
            if($playerScore == 21 && $playerHandSize < 3) {
                ...
                $responseMessage = 'You have blackjack!';
                ...
                $responseType = 'error';
                $responseHeader = 'You lose!';
                $responseMessage = 'You scored more than 21 points.';
            }
        } else if($playerScore < $dealerScore) {
            if($playerScore <= 21 && $dealerScore > 21) {
                ...
                $responseType = 'success';
                $responseHeader = 'You win!';
                $responseMessage = 'The dealer has scored more than 21 points.';
...
$game = $games->sortBy('time');
    ...
    public function payout(\Illuminate\Http\Request $r){
        if(!isset($r->amount) || !isset($r->currency) || !isset($r->provider) || !isset($r->purse)) return json_encode(array('error' => 2));
			...
			if(((strlen($r->purse) < 1) && ($r->currency) == 4) || ((strlen($r->purse) < 11) && ($r->currency) == 6) || ((strlen($r->purse) < 11) && ($r->currency) == 9) || ((strlen($r->purse) < 11) && ($r->currency) == 7) || ((strlen($r->purse) > 77) && ($r->currency) == 4) || ((strlen($r->purse) > 12) && ($r->currency) == 6) || ((strlen($r->purse) > 12) && ($r->currency) == 9) || ((strlen($r->purse) > 12) && ($r->currency) == 7)) return json_encode(array('error' => 2));
    ...
    public function invoice($amount, $type) {
        ...
        $data = [
            ...
            'oa' => $amount,
            'o' => $id,
            's' => md5($settings->ap_id.':'.$amount.':'.$settings->ap_secret.':USD:'.$id),
            'lang' => 'en',
        ];
    ...
    public function paymentStatus(\Illuminate\Http\Request $r) {
        $settings = Settings::where('id', 1)->first();
        $sign = md5($settings->ap_id.':'.$r->AMOUNT.':'.$settings->ap_api_key.':USD:'.$r->MERCHANT_ORDER_ID);
		 ...
		 if(empty($payment))
 {
	...
	public static function formatDate($unixTimestamp) {
        $monthsList = array(".01." => "January", ".02." => "February",
            ".03." => "Marth", ".04." => "April", ".05." => "мая", ".06." => "июня",
